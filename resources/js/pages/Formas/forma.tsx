...
export default function Anketa({ form }: { form: FormData | null }) {
  ...
  const getOptions = (options?: { lv: string[]; en: string[] }) =>
    options ? options[lang] || options.lv || [] : [];
  ...
  const handleCheckboxChange = (id: string, option: string) => {
    ...
    if (current.includes(option)) handleChange(id, current.filter((o) => o !== option));
      ...
      <div className="mx-auto w-full max-w-3xl space-y-8">
        ...
        <div className="rounded-[32px] border border-white/70 bg-white/90 p-8 text-center shadow-2xl shadow-slate-200/80">
          ...
          <p className="mt-3 text-sm text-slate-600">
            Aizpildiet soļus — jautājumiem nav pareizo vai nepareizo atbilžu.
          </p>
        </div>
        ...
        {submitted ? (
          ...
          <>
            ...
            <div className="space-y-6">
              {fields
                .slice(currentSlide * batchSize, currentSlide * batchSize + batchSize)
                .map((field) => {
              ...
              <div className="flex w-full flex-col gap-3 sm:w-auto sm:flex-row">
                ...
                {currentSlide < Math.ceil(fields.length / batchSize) - 1 ? (
                  <button
                    type="button"
                    onClick={() =>
                      setCurrentSlide((prev) => Math.min(prev + 1, Math.ceil(fields.length / batchSize) - 1))
                    }
