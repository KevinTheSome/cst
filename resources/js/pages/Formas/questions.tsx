    ...
    return (
        <>
            ...
            {showConsentModal && (
                <div className="fixed inset-0 z-40 flex items-center justify-center bg-slate-900/60 px-4">
                    <div className="w-full max-w-xl rounded-3xl border border-white/30 bg-white p-8 text-center shadow-2xl shadow-slate-900/40">
                        <p className="text-xs tracking-[0.4em] text-emerald-500 uppercase">Datu izmantošana</p>
                        <h2 className="mt-3 text-2xl font-semibold text-slate-900">Vai piekrītat datu apstrādei?</h2>
                        <p className="mt-3 text-sm text-slate-600">
                            Anketa apkopo veselības informāciju, lai sagatavotu personalizētu cilmes šūnu terapijas piedāvājumu. Ja nepiekrītat,
                            anketa netiks atvērta.
                        </p>
            ...
            <div className={showConsentModal ? 'pointer-events-none opacity-30 transition' : 'transition'}>
                <section className="bg-gradient-to-br from-[#f1f5f9] via-white to-[#e8f6ef] px-4 py-16 sm:px-6 lg:px-12">
                    <div className="mx-auto max-w-6xl space-y-10">
                        ...
                        <div className="relative overflow-hidden rounded-[32px] border border-white/60 bg-white/70 p-8 shadow-2xl shadow-emerald-100/80 backdrop-blur">
                            ...
                            <header className="relative text-center">
                                <p className="text-sm tracking-[0.3em] text-emerald-600 uppercase">Pieteikuma forma</p>
                                <h1 className="mt-3 text-3xl font-semibold text-slate-900 sm:text-4xl">Anketa pacientiem</h1>
                                <p className="mt-4 text-lg text-slate-700">
                                    Aizpildiet anketu – jautājumi tiek ielādēti dinamiski no sistēmas, izmantojot jūsu kodu.
                                </p>
                            </header>
                            ...
                            <div className="relative mt-10 grid gap-4 md:grid-cols-3">
                                {highlightCards.map((card) => (
                                    ...
                                    >
                                        <p className="text-sm font-semibold tracking-[0.2em] text-emerald-500 uppercase">{card.accent}</p>
                        ...
                        {!isCodeVerified && (
                            <div className="mx-auto max-w-4xl rounded-[28px] border border-emerald-100/80 bg-white/90 p-8 text-center shadow-xl shadow-emerald-100">
                                <p className="text-sm font-semibold tracking-[0.4em] text-emerald-500 uppercase">Droša piekļuve</p>
                                <h2 className="mt-3 text-2xl font-semibold text-slate-900">Atbloķējiet anketu ar 12 simbolu kodu</h2>
                                <p className="mt-3 text-base text-slate-600">
                                    Kodā ir iekļauti 12 cipari vai lielie burti. Ievadiet to blokos ērtākai pārbaudei un spiediet pogu, lai sāktu
                                    aizpildīt anketu.
                                </p>
                                ...
                                >
                                        ...
                                        {codeDigits.map((digit, index) => (
                                            <div key={index} className="flex items-center">
                                                <input
                                                    ...
                                                    disabled={verifying}
                                                    className="h-14 w-12 rounded-xl border border-slate-300 bg-white text-center text-xl font-semibold tracking-widest text-slate-900 transition outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200"
                                    ...
                                    <p className="text-sm text-slate-500">
                                        {charactersRemaining > 0 ? `Atlikuši ${charactersRemaining} simboli` : 'Kods gatavs iesniegšanai'}
                                    </p>
                        ...
                        {isCodeVerified && consentChoice !== 'pending' && (
                            <>
                                {consentChoice === 'declined' ? (
                                    <div className="rounded-3xl bg-white p-12 text-center shadow-2xl ring-1 shadow-emerald-100/60 ring-emerald-100">
                                        <p className="text-sm tracking-[0.4em] text-emerald-600 uppercase">Piekrišana atteikta</p>
                                        <h2 className="mt-4 text-3xl font-semibold text-slate-900">Anketa nav pieejama</h2>
                                        <p className="mt-3 text-sm text-slate-600">
                                            Datu apstrāde ir nepieciešama, lai sagatavotu personalizētu piedāvājumu.
                                        </p>
                                    ...
                                    <>
                                        {/* Dynamic anketa form */}
                                        {isCodeVerified && loadedForm && !submitted && (
                                            <form
                                                onSubmit={handleSubmitAnswers}
                                                className="space-y-8 rounded-3xl bg-white/95 p-8 shadow-2xl ring-1 shadow-emerald-100/60 ring-slate-100"
                                            >
                                                <div className="space-y-2">
                                                    <p className="text-sm tracking-[0.3em] text-emerald-600 uppercase">Anketa</p>
                                                    <h2 className="text-2xl font-semibold text-slate-900">
                                                        {typeof loadedForm.title === 'object'
                                                            ? (loadedForm.title[activeLang] ?? loadedForm.title.lv ?? 'Anketa')
                                                            : loadedForm.title || 'Anketa'}
                                                    </h2>
                                                    <p className="text-sm text-slate-600">Aizpildiet jautājumus un iesniedziet atbildes.</p>
                                                </div>
                                                ...
                                                <div className="space-y-6">
                                                    {loadedForm.fields.length === 0 && (
                                                        <p className="text-sm text-slate-500 italic">Šai anketai nav definētu jautājumu.</p>
                                                    )}

                                                    {loadedForm.fields.map((field, index) => {
                                                        const label = (field.label?.[activeLang] as string) ?? (field.label?.lv as string) ?? '';

                                                        const options =
                                                            (field.options?.[activeLang] as string[] | undefined) ??
                                                            (field.options?.lv as string[] | undefined) ??
                                                            [];
                                                        ...
                                                        return (
                                                            ...
                                                            >
                                                                        ...
                                                                        {options.map((opt) => (
                                                                            ...
                                                                            >
                                                                                <input
                                                                                    ...
                                                                                    checked={answer === opt}
                                                                                    onChange={() => handleRadioAnswer(field.id, opt)}
                                                                                />
                                                                                <span>{opt}</span>
                                                                            </label>
                                                                        ))}
                                                                            ...
                                                                            return (
                                                                                ...
                                                                                >
                                                                                    <input
                                                                                        ...
                                                                                        checked={checked}
                                                                                        onChange={() => handleCheckboxAnswer(field.id, opt)}
                                                                                    />
                                                                                    <span>{opt}</span>
                                                                                </label>
                                                                            );
                                                                ...
                                                                {field.type === 'dropdown' && (
                                                                    <select
                                                                        className="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-900 transition outline-none focus:border-emerald-600 focus:ring-2 focus:ring-emerald-100"
                                                                        value={typeof answer === 'string' ? answer : ''}
                                                                        onChange={(e) => handleDropdownAnswer(field.id, e.target.value)}
                                                                    >
                                                                ...
                                                                {field.type === 'text' && (
                                                                    <input
                                                                        ...
                                                                        className="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-900 transition outline-none focus:border-emerald-600 focus:ring-2 focus:ring-emerald-100"
                                                                        placeholder={
                                                                            (field.placeholder?.[activeLang] as string) ??
                                                                            (field.placeholder?.lv as string) ??
                                                                            ''
                                                                        }
                                                                        value={typeof answer === 'string' ? answer : ''}
                                                                        onChange={(e) => handleTextAnswer(field.id, e.target.value)}
                                                                    />
                                                                )}
                                                                ...
                                                                {field.type === 'scale' && field.scale && (
                                                                    <div className="space-y-4">
                                                                        <div className="flex justify-between text-xs text-slate-500">
                                                                            <span>
                                                                                {(field.scale.minLabel?.[activeLang] as string) ??
                                                                                    (field.scale.minLabel?.lv as string) ??
                                                                                    `${field.scale.min}`}
                                                                            </span>
                                                                            <span>
                                                                                {(field.scale.maxLabel?.[activeLang] as string) ??
                                                                                    (field.scale.maxLabel?.lv as string) ??
                                                                                    `${field.scale.max}`}
                                                                            </span>
                                                                        </div>
                                                                        <div className="flex gap-2">
                                                                            {Array.from(
                                                                                ...
                                                                                <button
                                                                                    ...
                                                                                    onClick={() => handleScaleAnswer(field.id, String(value))}
                                                                                >
                                                                                    {value}
                                                                                </button>
                                                                            ))}
                                        ...
                                        {isCodeVerified && loadedForm && submitted && (
                                            <div className="rounded-3xl bg-white p-12 text-center shadow-2xl ring-1 shadow-emerald-100/60 ring-emerald-100">
                                                <p className="text-sm tracking-[0.4em] text-emerald-600 uppercase">Paldies!</p>
                                                <h2 className="mt-4 text-3xl font-semibold text-slate-900">Jūsu atbildes ir saglabātas</h2>
                                            </div>
                                        )}
                                    </>
                                )}
