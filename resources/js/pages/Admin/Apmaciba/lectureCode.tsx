...
export default function LectureCode() {
    ...
    const findTrainingTitle = (t?: OnlineTraining) => {
        if (!t) return __('lecture_codes.misc.dash');
        ...
        const title =
            ...
            t.title?.lv ||
            t.title?.en ||
            (Array.isArray(t.title) ? t.title[0] : null) ||
            null;

        return title ?? `${__('lecture_codes.misc.training')} #${t.id}`;
    };

    const humanDate = (s?: string | null) => (s ? new Date(s).toLocaleString() : __('lecture_codes.misc.dash'));
    ...
    const handleCreate = async (e?: React.FormEvent) => {
        ...
        if (newCodeData.max_uses < 0) {
            setFormError(__('lecture_codes.errors.max_uses_non_negative'));
            return;
        }
        ...
        try {
            ...
            if (newCodeData.online_training_ids.length === 1) payload.online_training_id = newCodeData.online_training_ids[0];
            else payload.online_training_id = null;
            ...
            setNewCodeData({ code: '', online_training_ids: [], max_uses: 0, is_active: true });
            setAlertState({ type: 'success', message: __('lecture_codes.alerts.created') });
        } catch (err: any) {
            setFormError(err?.response?.data?.message || __('lecture_codes.errors.create_failed'));
        } finally {
            setCreating(false);
        }
    };

    const handleDeleteRequest = (id: number, codeName: string) => setConfirmState({ type: 'delete', id, codeName });
    const handleRegenerateRequest = (id: number, codeName: string) => setConfirmState({ type: 'regenerate', id, codeName });
    ...
    const proceedWithAction = async () => {
        ...
        try {
            ...
            if (confirmState.type === 'delete') {
                ...
                setAlertState({ type: 'success', message: __('lecture_codes.alerts.deleted') });
            } else {
                ...
                setAlertState({ type: 'success', message: __('lecture_codes.alerts.regenerated') });
            }

            setConfirmState(null);
        } catch {
            setAlertState({ type: 'error', message: __('lecture_codes.alerts.action_failed') });
        } finally {
    ...
    const handleUpdate = async (e?: React.FormEvent) => {
        ...
        try {
            ...
            if ((editing.online_training_ids ?? []).length === 1) payload.online_training_id = (editing.online_training_ids ?? [])[0];
            else payload.online_training_id = null;
            ...
            setAlertState({ type: 'success', message: __('lecture_codes.alerts.updated') });
        } catch (err: any) {
            setEditError(err?.response?.data?.message || __('lecture_codes.errors.update_failed'));
        } finally {
            setSubmitting(false);
        }
    };
    ...
    const renderTrainingsForCode = (c: OnlineCode) => {
        ...
        if (!ids.length) return __('lecture_codes.misc.all_trainings');
    ...
    return (
        <>
            <Head title={__('lecture_codes.page_title')} />

            <div className="mx-auto w-full max-w-7xl space-y-8 p-4 md:p-8">
                {/* Header */}
                <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                    <div>
                        <h1 className="text-3xl font-bold tracking-tight text-white">{__('lecture_codes.heading')}</h1>
                        <p className="mt-1 text-slate-400">{__('lecture_codes.subheading')}</p>
                    </div>
                </div>
                        ...
                        <form onSubmit={handleCreate} className="grid grid-cols-1 gap-6 md:grid-cols-12">
                            <div className="flex flex-col gap-2 md:col-span-4">
                                <label className="text-xs font-medium tracking-wider text-slate-400 uppercase">
                                    {__('lecture_codes.create.custom_code_label')}
                                </label>
                                <input
                                    value={newCodeData.code}
                                    onChange={(e) => setNewCodeData((s) => ({ ...s, code: e.target.value }))}
                                    placeholder={__('lecture_codes.create.custom_code_placeholder')}
                            ...
                            <div className="flex flex-col gap-2 md:col-span-5">
                                                    ...
                                                    >
                                                        <div
                                                            className={`flex h-5 w-5 items-center justify-center rounded border transition-colors ${
                                                                checked ? 'border-emerald-500 bg-emerald-500 text-black' : 'border-slate-600 bg-transparent'
                                                            }`}
                                                        >
                                                            {checked && (
                                                            ...
                                                            <input type="checkbox" checked={checked} onChange={() => toggleCreateTraining(t.id)} className="hidden" />
                                                        </div>
                                ...
                                <p className="pl-1 text-[10px] text-slate-500">
                                    {__('lecture_codes.create.selected')}: {newCodeData.online_training_ids.length ? newCodeData.online_training_ids.length : __('lecture_codes.misc.all_universal')}
                                </p>
                            </div>

                            <div className="flex flex-col gap-2 md:col-span-3">
                                <label className="text-xs font-medium tracking-wider text-slate-400 uppercase">
                                    {__('lecture_codes.create.max_uses_label')}
                                </label>
                                <input
                                    type="number"
                                    min={0}
                                ...
                                />
                                <p className="pl-1 text-[10px] text-slate-500">{__('lecture_codes.create.max_uses_hint')}</p>
                            </div>
                            ...
                            <div className="mt-2 flex flex-col items-center justify-between gap-4 border-t border-white/5 pt-4 md:col-span-12 md:flex-row">
                                ...
                                >
                                    {creating ? __('lecture_codes.create.creating') : __('lecture_codes.create.create_button')}
                                </button>
                            </div>
                        </form>
                    </div>
                ...
                <div className="overflow-hidden rounded-3xl border border-white/10 bg-slate-900/50 shadow-2xl backdrop-blur-xl">
                    <div className="flex items-center justify-between border-b border-white/5 px-6 py-5">
                        <h3 className="text-lg font-semibold text-white">{__('lecture_codes.table.title')}</h3>
                        <span className="rounded-md bg-white/5 px-2 py-1 font-mono text-xs text-slate-500">
                            {codes.length} {__('lecture_codes.table.records')}
                        </span>
                    </div>
                        ...
                        <table className="w-full border-collapse text-left">
                            <thead>
                                <tr className="bg-white/5 text-xs font-bold tracking-wider text-slate-400 uppercase">
                                    <th className="px-6 py-4 whitespace-nowrap">{__('lecture_codes.table.th_code_date')}</th>
                                    <th className="px-6 py-4 whitespace-nowrap">{__('lecture_codes.table.th_scope')}</th>
                                    <th className="px-6 py-4 text-center whitespace-nowrap">{__('lecture_codes.table.th_usage')}</th>
                                    <th className="px-6 py-4 text-center whitespace-nowrap">{__('lecture_codes.table.th_status')}</th>
                                    <th className="px-6 py-4 text-right whitespace-nowrap">{__('lecture_codes.table.th_actions')}</th>
                                </tr>
                            </thead>
                                    ...
                                    <tr key={c.id} className="group transition-colors hover:bg-white/[0.02]">
                                        <td className="px-6 py-4">
                                            <div className="flex flex-col">
                                                <span className="font-mono text-base font-medium text-emerald-400 transition-colors group-hover:text-emerald-300">{c.code}</span>
                                        ...
                                        <td className="px-6 py-4 text-center">
                                            <div className="inline-flex items-center gap-1 rounded-md border border-white/5 bg-white/5 px-2.5 py-1 text-xs font-medium text-slate-300">
                                                <span className={c.used_count >= c.max_uses && c.max_uses !== 0 ? 'text-rose-400' : 'text-white'}>{c.used_count}</span>
                                        ...
                                        <td className="px-6 py-4 text-center">
                                            <span className={`inline-flex items-center rounded-full border px-2.5 py-0.5 text-xs font-medium ${
                                                c.is_active ? 'border-emerald-500/20 bg-emerald-500/10 text-emerald-400' : 'border-slate-700 bg-slate-800 text-slate-400'
                                            }`}>
                                                {c.is_active ? __('lecture_codes.status.active') : __('lecture_codes.status.inactive')}
                                            </span>
                                        </td>

                                        <td className="px-6 py-4 text-right whitespace-nowrap">
                                            <div className="flex items-center justify-end gap-2 opacity-80 transition-opacity group-hover:opacity-100">
                                                <button type="button" onClick={() => openEdit(c)} className="rounded-lg bg-slate-800 p-2 text-slate-300 transition-colors hover:bg-slate-700 hover:text-white" title={__('lecture_codes.actions.edit')}>
                                                    <IconEdit />
                                                </button>
                                                <button type="button" onClick={() => handleRegenerateRequest(c.id, c.code)} className="rounded-lg bg-slate-800 p-2 text-blue-400 transition-colors hover:bg-blue-500/20 hover:text-blue-300" title={__('lecture_codes.actions.regenerate')}>
                                                    <IconRefresh />
                                                </button>
                                                <button type="button" onClick={() => handleDeleteRequest(c.id, c.code)} className="rounded-lg bg-slate-800 p-2 text-rose-400 transition-colors hover:bg-rose-500/20 hover:text-rose-300" title={__('lecture_codes.actions.delete')}>
                                                    <IconTrash />
                                                </button>
                                            </div>
                                        </td>
                                ...
                                {codes.length === 0 && (
                                    <tr>
                                        <td colSpan={5} className="px-6 py-12 text-center text-slate-500">
                                            <p className="text-base">{__('lecture_codes.empty.title')}</p>
                                            <p className="text-sm">{__('lecture_codes.empty.subtitle')}</p>
                                        </td>
                                    </tr>
                                )}
                            </tbody>
                ...
                {confirmState && (
                    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
                        <div className="absolute inset-0 bg-black/80 backdrop-blur-sm transition-opacity" onClick={() => !processingAction && setConfirmState(null)} />
                        ...
                        <div className="animate-in fade-in zoom-in relative w-full max-w-sm overflow-hidden rounded-2xl border border-white/10 bg-slate-900 shadow-2xl duration-200">
                            <div className="p-6 text-center">
                                ...
                                <h3 className="mb-2 text-xl font-bold text-white">
                                    {confirmState.type === 'delete' ? __('lecture_codes.confirm.delete_title') : __('lecture_codes.confirm.regenerate_title')}
                                </h3>

                                <p className="mb-6 text-sm text-slate-400">
                                    {confirmState.type === 'delete'
                                        ? __('lecture_codes.confirm.delete_text', { code: confirmState.codeName ?? '' })
                                        : __('lecture_codes.confirm.regenerate_text', { code: confirmState.codeName ?? '' })}
                                </p>
                                ...
                                <div className="flex gap-3">
                                    ...
                                    >
                                        {__('lecture_codes.actions.cancel')}
                                    </button>
                                    <button
                                        disabled={processingAction}
                                    ...
                                    >
                                        {processingAction
                                            ? __('lecture_codes.actions.processing')
                                            : confirmState.type === 'delete'
                                            ? __('lecture_codes.actions.confirm_delete')
                                            : __('lecture_codes.actions.confirm_regenerate')}
                                    </button>
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="pointer-events-none fixed inset-0 z-[70] flex items-center justify-center p-4">
                        <div className="animate-in slide-in-from-bottom-10 fade-in pointer-events-auto relative w-full max-w-sm rounded-2xl border border-white/10 bg-slate-900 p-6 shadow-2xl duration-300">
                            <div className="flex flex-col items-center text-center">
                                ...
                                <h3 className="mb-1 text-lg font-bold text-white">{alertState.type === 'success' ? __('lecture_codes.alerts.success') : __('lecture_codes.alerts.error')}</h3>
                                <p className="mb-6 text-sm text-slate-400">{alertState.message}</p>
                                <button onClick={() => setAlertState(null)} className="w-full rounded-xl bg-slate-800 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-slate-700">
                                    {__('lecture_codes.actions.ok')}
                                </button>
                            </div>
                        </div>
                    </div>
                ...
                {editing && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                        ...
                        <div className="animate-in fade-in zoom-in relative w-full max-w-xl rounded-2xl border border-white/10 bg-slate-900 p-8 shadow-2xl duration-200">
                            <h3 className="mb-6 text-xl font-bold text-white">{__('lecture_codes.edit.title')}</h3>

                            <form onSubmit={handleUpdate} className="space-y-6">
                                <div className="space-y-2">
                                    <label className="text-xs font-medium tracking-wider text-slate-400 uppercase">{__('lecture_codes.edit.code_label')}</label>
                                    <input
                                        value={editing.code ?? ''}
                                ...
                                <div className="space-y-2">
                                    <label className="text-xs font-medium tracking-wider text-slate-400 uppercase">{__('lecture_codes.edit.trainings_label')}</label>
                                    <div className="custom-scrollbar h-[150px] overflow-y-auto rounded-xl border border-white/10 bg-black/40 p-2">
                                        {trainings.length === 0 && <div className="p-2 text-xs text-white/50">{__('lecture_codes.edit.no_trainings')}</div>}
                                        {trainings.map((t) => {
                                            const checked = (editing.online_training_ids ?? []).includes(t.id);
                                            return (
                                                <label key={t.id} className={`flex cursor-pointer items-center gap-3 rounded-lg px-3 py-2 transition-colors ${checked ? 'bg-emerald-500/10' : 'hover:bg-white/5'}`}>
                                                    <div className={`flex h-4 w-4 items-center justify-center rounded border transition-colors ${checked ? 'border-emerald-500 bg-emerald-500 text-black' : 'border-slate-600 bg-transparent'}`}>
                                                        {checked && (
                                                            <svg className="h-3 w-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="3">
                                                                <path d="M5 13l4 4L19 7"></path>
                                                            </svg>
                                                        )}
                                                        <input type="checkbox" checked={checked} onChange={() => toggleEditTraining(t.id)} className="hidden" />
                                                    </div>
                                                    <span className={`text-sm ${checked ? 'text-emerald-400' : 'text-slate-300'}`}>{findTrainingTitle(t)}</span>
                                                </label>
                                            );
                                        })}
                                    </div>
                                ...
                                <div className="space-y-2">
                                    <label className="text-xs font-medium tracking-wider text-slate-400 uppercase">{__('lecture_codes.edit.max_uses_label')}</label>
                                    <input
                                        type="number"
                                        min="0"
                                ...
                                <div className="flex gap-3 pt-2">
                                    <button disabled={submitting} type="submit" className="flex-1 rounded-xl bg-emerald-500 px-4 py-3 font-bold text-slate-900 transition-colors hover:bg-emerald-400">
                                        {submitting ? __('lecture_codes.edit.saving') : __('lecture_codes.edit.save')}
                                    </button>
                                    <button type="button" onClick={() => setEditing(null)} className="flex-1 rounded-xl border border-white/10 bg-transparent px-4 py-3 font-semibold text-white transition-colors hover:bg-white/5">
                                        {__('lecture_codes.actions.cancel')}
                                    </button>
                                </div>
                            </form>
...
// ✅ Layout title must use page props (no hooks here)
(LectureCode as any).layout = (page: any) => {
    const t =
        page?.props?.lang?.lecture_codes?.layout_title ||
        page?.props?.lang?.lecture_codes?.page_title ||
        'Lecture codes';
