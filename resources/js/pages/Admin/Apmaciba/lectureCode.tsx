      ...
      <div className="w-full max-w-7xl mx-auto space-y-8 p-4 md:p-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            ...
            <p className="text-slate-400 mt-1">Manage validation codes for online course access.</p>
          </div>
        </div>
        ...
        <div className="relative overflow-hidden rounded-3xl border border-white/10 bg-slate-900/50 p-6 shadow-2xl backdrop-blur-xl transition-all">
          <div className="absolute -top-24 -right-24 h-64 w-64 rounded-full bg-emerald-500/10 blur-3xl pointer-events-none"></div>
          ...
          <div className="relative z-10">
            ...
            <form onSubmit={handleCreate} className="grid grid-cols-1 md:grid-cols-12 gap-6">
              ...
              <div className="md:col-span-4 flex flex-col gap-2">
                <label className="text-xs font-medium uppercase tracking-wider text-slate-400">Custom Code (Optional)</label>
                <div className="relative">
                    <input
                        value={newCodeData.code}
                        onChange={(e) => setNewCodeData((s) => ({ ...s, code: e.target.value }))}
                        placeholder="Leave empty to auto-generate"
                        className="w-full rounded-xl border border-white/10 bg-black/20 px-4 py-3 text-white placeholder-white/30 transition-all focus:border-emerald-500/50 focus:bg-black/40 focus:outline-none focus:ring-2 focus:ring-emerald-500/20"
                    />
                </div>
              </div>

              <div className="md:col-span-5 flex flex-col gap-2">
                <label className="text-xs font-medium uppercase tracking-wider text-slate-400">Applicable Trainings</label>
                      ...
                      {trainings.map((t) => {
                        ...
                        return (
                          <label key={t.id} className={`flex cursor-pointer items-center gap-3 rounded-lg px-3 py-2 transition-colors ${checked ? 'bg-emerald-500/10' : 'hover:bg-white/5'}`}>
                            <div className={`flex h-5 w-5 items-center justify-center rounded border transition-colors ${checked ? 'border-emerald-500 bg-emerald-500 text-black' : 'border-slate-600 bg-transparent'}`}>
                                {checked && <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="3"><path d="M5 13l4 4L19 7"></path></svg>}
                                <input type="checkbox" checked={checked} onChange={() => toggleCreateTraining(t.id)} className="hidden" />
                            </div>
                            <span className={`text-sm ${checked ? 'text-emerald-400 font-medium' : 'text-slate-300'}`}>{findTrainingTitle(t)}</span>
                          </label>
                        );
                      })}
                    </div>
                ...
                <p className="text-[10px] text-slate-500 pl-1">Selected: {newCodeData.online_training_ids.length > 0 ? newCodeData.online_training_ids.length : 'All (Universal Access)'}</p>
              </div>

              <div className="md:col-span-3 flex flex-col gap-2">
                <label className="text-xs font-medium uppercase tracking-wider text-slate-400">Max Uses</label>
                <div className="flex items-center gap-2">
                    <input
                        ...
                        className="w-full rounded-xl border border-white/10 bg-black/20 px-4 py-3 text-white transition-all focus:border-emerald-500/50 focus:bg-black/40 focus:outline-none focus:ring-2 focus:ring-emerald-500/20"
                    />
                </div>
                <p className="text-[10px] text-slate-500 pl-1">Set to 0 for unlimited uses.</p>
              </div>

              <div className="md:col-span-12 flex flex-col md:flex-row items-center justify-between gap-4 border-t border-white/5 pt-4 mt-2">
                <div className="text-xs text-slate-400">
                    {formError && <span className="text-rose-400 font-medium bg-rose-400/10 px-2 py-1 rounded-lg">{formError}</span>}
                </div>
                <button
                  type="submit"
                  disabled={creating}
                  className={`w-full md:w-auto rounded-xl bg-gradient-to-r from-emerald-500 to-emerald-400 px-8 py-3 text-sm font-bold text-slate-900 shadow-lg shadow-emerald-500/20 transition-all hover:scale-[1.02] hover:shadow-emerald-500/30 focus:outline-none focus:ring-2 focus:ring-emerald-500/40 disabled:opacity-70 disabled:hover:scale-100 ${creating ? 'cursor-wait' : ''}`}
                >
        ...
        <div className="rounded-3xl border border-white/10 bg-slate-900/50 overflow-hidden shadow-2xl backdrop-blur-xl">
            <div className="px-6 py-5 border-b border-white/5 flex items-center justify-between">
                ...
                <span className="text-xs font-mono text-slate-500 bg-white/5 px-2 py-1 rounded-md">{codes.length} records</span>
            </div>
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-white/5 text-xs font-bold uppercase tracking-wider text-slate-400">
                    ...
                    <th className="px-6 py-4 whitespace-nowrap text-center">Usage</th>
                    ...
                    <tr key={c.id} className="group transition-colors hover:bg-white/[0.02]">
                      <td className="px-6 py-4">
                        <div className="flex flex-col">
                            <span className="font-mono text-base font-medium text-emerald-400 group-hover:text-emerald-300 transition-colors">{c.code}</span>
                      ...
                      <td className="px-6 py-4 text-center">
                        <div className="inline-flex items-center gap-1 rounded-md bg-white/5 px-2.5 py-1 text-xs font-medium text-slate-300 border border-white/5">
                      ...
                      <td className="px-6 py-4 text-slate-400 text-xs whitespace-nowrap">
                        <div className="flex flex-col gap-0.5">
                            <span>From: {c.valid_from ? new Date(c.valid_from).toLocaleDateString() : 'Anytime'}</span>
                            <span>To: <span className="ml-3">{c.valid_until ? new Date(c.valid_until).toLocaleDateString() : 'Forever'}</span></span>
                        </div>
                      </td>
                      <td className="px-6 py-4 text-center">
                      ...
                      <td className="px-6 py-4 text-right whitespace-nowrap">
                        <div className="flex items-center justify-end gap-2 opacity-80 group-hover:opacity-100 transition-opacity">
                          ...
                          <button 
                            ...
                            className="rounded-lg bg-slate-800 p-2 text-blue-400 hover:bg-blue-500/20 hover:text-blue-300 transition-colors"
                          ...
                          <button
                            ...
                            className="rounded-lg bg-slate-800 p-2 text-rose-400 hover:bg-rose-500/20 hover:text-rose-300 transition-colors"
                            title="Delete"
                          >
                            <IconTrash />
                          </button>
        ...
        {alertState && (
            <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 pointer-events-none">
                 ...
                 <div className="pointer-events-auto relative w-full max-w-sm rounded-2xl border border-white/10 bg-slate-900 p-6 shadow-2xl animate-in slide-in-from-bottom-10 fade-in duration-300">
                    <div className="flex flex-col items-center text-center">
                        <div className="mb-4">
                            {alertState.type === 'success' ? <IconCheck /> : <IconWarning />}
                        </div>
                        <h3 className="text-lg font-bold text-white mb-1">
                            {alertState.type === 'success' ? 'Success' : 'Error'}
                        </h3>
                        <p className="text-sm text-slate-400 mb-6">{alertState.message}</p>
                        <button 
        ...
        {editing && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div className="relative w-full max-w-xl rounded-2xl border border-white/10 bg-slate-900 p-8 shadow-2xl animate-in fade-in zoom-in duration-200">
              <h3 className="text-xl font-bold text-white mb-6">Edit Access Code</h3>

              <form onSubmit={handleUpdate} className="space-y-6">
                <div className="space-y-2">
                  <label className="text-xs font-medium uppercase tracking-wider text-slate-400">Code String</label>
                  <input
                    value={editing.code ?? ''}
                    onChange={(e) => setEditing((s) => ({ ...(s ?? {}), code: e.target.value }))}
                    className="w-full rounded-xl border border-white/10 bg-black/40 px-4 py-3 text-white focus:border-emerald-500/50 focus:outline-none focus:ring-1 focus:ring-emerald-500/50"
                  />
                </div>

                <div className="space-y-2">
                  <label className="text-xs font-medium uppercase tracking-wider text-slate-400">Assigned Trainings</label>
                      ...
                      return (
                        <label key={t.id} className={`flex cursor-pointer items-center gap-3 rounded-lg px-3 py-2 transition-colors ${checked ? 'bg-emerald-500/10' : 'hover:bg-white/5'}`}>
                            <div className={`flex h-4 w-4 items-center justify-center rounded border transition-colors ${checked ? 'border-emerald-500 bg-emerald-500 text-black' : 'border-slate-600 bg-transparent'}`}>
                                {checked && <svg className="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="3"><path d="M5 13l4 4L19 7"></path></svg>}
                ...
                <div className="flex gap-3 pt-2">
                  <button disabled={submitting} type="submit" className="flex-1 rounded-xl bg-emerald-500 px-4 py-3 font-bold text-slate-900 hover:bg-emerald-400 transition-colors">
                    Save Changes
                  </button>
                  <button type="button" onClick={() => setEditing(null)} className="flex-1 rounded-xl border border-white/10 bg-transparent px-4 py-3 font-semibold text-white hover:bg-white/5 transition-colors">
                    Cancel
                  </button>
                </div>
              </form>
