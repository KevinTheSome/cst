...
export default function CreateAnketa() {
  ...
  // Add default *option* question (radio) — admin can switch type after adding
  const addField = () => {
  ...
  // Change field type and replace data with sensible defaults for that type
  ...
  // Option editors (only for OptionField)
    ...
    setFields((prev) =>
      prev.map((f) => {
        ...
        if (f.type === 'radio' || f.type === 'checkbox' || f.type === 'dropdown') {
          ...
          return { ...f, options: { lv: [...f.options.lv, `Opcija ${nextIndex}`], en: [...f.options.en, `Option ${nextIndex}`] } } as OptionField;
        }
        return f;
      })
    );
  ...
  setFields((prev) =>
    prev.map((f) => {
      ...
      if (f.type === 'scale') {
        ...
        if (key === 'min' || key === 'max') {
          // force integer and clamp between 1 and 10
          ...
          // ensure min < max; nudge the other side if needed
          if (s.min >= s.max) {
