...
type VerifyResponse = {
  valid: boolean;
  message?: string;
  // backend may return lectures as full objects; we only need ids
...
export default function OnlineTraining({
  ...
  // Ensure axios has csrf headers (usually already, but safe)
  useEffect(() => {
  ...
  const handleSubmitCode = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      // ✅ fallback: if backend doesn't return lectures, still unlock the one the user clicked
      if (ids.length === 0) {
        ids.push(selectedLectureId);
      }
      ...
      setBanner({ type: 'success', text: `Unlocked training IDs: ${ids.join(', ')}` });
  ...
  const handleSubmitRating = async (lectureId: number) => {
    ...
    try {
      await axios.post('/ratings', { lectureId, rating });
      ...
      >
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            {/* Always show unlocked state */}
            {unlocked && (
              <div className="mt-2 inline-flex items-center gap-1 rounded-full border border-emerald-200 bg-emerald-100 px-2 py-0.5 text-[11px] font-semibold text-emerald-700">
              ...
              <div className="mt-2 flex items-center gap-2 text-xs text-amber-600">
                {[1, 2, 3, 4, 5].map((i) => (
                  <svg
                    key={i}
                    className={`h-3 w-3 ${
                      i <= Math.round(lecture.rating_avg!) ? 'fill-amber-400' : 'fill-transparent stroke-amber-400'
                    }`}
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                    strokeWidth="2"
          ...
          <div className="flex flex-col items-center gap-2">
            {unlocked && lecture.url ? (
              ...
              <div
                className="flex h-10 w-10 items-center justify-center rounded-full border border-emerald-200 bg-emerald-100 text-emerald-700"
                title="Unlocked (no URL)"
              >
